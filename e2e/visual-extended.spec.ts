...
test.describe('Extended Visual Regression', () => {
	...
	test('Game - Win State', async ({ page }) => {
		...
		await argosScreenshot(page, 'extended-game-win-light');

		await setTheme(page, 'dark');
		await argosScreenshot(page, 'extended-game-win-dark');
	});
	...
	test('Game - Failure State', async ({ page }) => {
		...
		// Screenshot
		await setTheme(page, 'light');
		await argosScreenshot(page, 'extended-game-fail-light');
	});
	...
	test('Builder - Interaction', async ({ page }) => {
		...
		// Wait for the cell to update (it should have a wall class or child)
		// Assuming .grid-cell has a child or class when occupied.
		// Let's just wait a tiny bit or check for visual change via screenshot.
		await page.waitForTimeout(200);

		// Screenshot with a wall placed
		await setTheme(page, 'light');
		await argosScreenshot(page, 'extended-builder-with-wall-light');
	});
});
