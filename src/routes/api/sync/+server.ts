...
const SyncUpdateSchema = z.object({
	levelId: z.string().min(1).max(200),
	stars: z.number().int().min(0).max(3),
	status: z.enum(['locked', 'unlocked', 'completed'])
});

const SyncPayloadSchema = z.object({
	updates: z.array(SyncUpdateSchema).max(500)
});

type SyncStatus = z.infer<typeof SyncUpdateSchema>['status'];
...
function isSyncStatus(status: string): status is SyncStatus {
	return status in STATUS_WEIGHT;
}

function getStatusWeight(status: string) {
	return isSyncStatus(status) ? STATUS_WEIGHT[status] : 0;
}
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const payload = SyncPayloadSchema.safeParse(await request.json().catch(() => null));
	if (!payload.success) {
	...
	const syncedState: { levelId: string; stars: number; status: SyncStatus }[] = [];

	for (const update of updates) {
		const current = existingMap.get(update.levelId);
		let finalStars = update.stars;
		let finalStatus: SyncStatus = update.status;

		if (current) {
			// High Water Mark Logic
			finalStars = Math.max(current.stars, update.stars);
			if (
				isSyncStatus(current.status) &&
				getStatusWeight(current.status) > getStatusWeight(update.status)
			) {
