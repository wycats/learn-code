...
import { LevelPackSchema } from '$lib/game/schema';
import { safeSlug } from '$lib/server/security';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const result = LevelPackSchema.safeParse(await request.json().catch(() => null));
	if (!result.success) {
		return json({ error: 'Invalid pack payload' }, { status: 400 });
	}
	...
	const sanitizedName = safeSlug(pack.name, 'pack');
	...
	for (const [index, level] of pack.levels.entries()) {
		...
		const levelFilename = `${prefix}-${safeSlug(level.name, 'level')}.json`;
		files.push({
