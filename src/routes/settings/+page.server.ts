...
import { revokeDeviceForUser } from '$lib/server/device-auth';
...
export const actions: Actions = {
	...
	deleteProfile: async (event) => {
		...
		await db
			...
			.where(and(eq(table.profile.id, profileId), eq(table.profile.userId, event.locals.user.id)));
	...
	revokeDevice: async (event) => {
		...
		await revokeDeviceForUser(deviceId, event.locals.user.id);
