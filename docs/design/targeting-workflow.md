...
The current "Pick Mode" for selecting targets is confusing because it lacks context and affordances. Users are dropped into a mode where they click things, but it's unclear _what_ they are populating or how to manage the selection (e.g., clearing it).
...
- **LevelDefinition**:
  ...
  - This allows us to decouple the _concept_ of a target from the _implementation_ (UUID).
...
- **Clarity**: The Banner/Bar explicitly states _what_ is happening ("Targeting Mode").
